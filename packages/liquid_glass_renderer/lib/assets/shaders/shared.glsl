...
vec3 toLin(vec3 s) {
    return vec3(_srgb_to_linear(s.r), _srgb_to_linear(s.g), _srgb_to_linear(s.b));
}

vec3 toSR(vec3 l) {
    return vec3(_linear_to_srgb(l.r), _linear_to_srgb(l.g), _linear_to_srgb(l.b));
}
#else
vec3 toLin(vec3 s) {
...
vec3 toSR(vec3 l) {
    return pow(max(l, vec3(0.0)), vec3(1.0 / 2.2));
}
#endif
...
#if SAMPLER_CLAMP
vec4 texScreen(sampler2D t, vec2 uv) {
    return texture(t, clamp(uv, vec2(0.0), vec2(1.0)));
}
#else
vec4 texScreen(sampler2D t, vec2 uv) {
    return texture(t, mirror01(uv));
}
#endif
...
// ÄNDERUNG: childTexture + childUVBase
vec4 calculateRefraction(
    ...
    sampler2D childTexture, // NEU
    vec2 childUVBase,       // NEU
    ...
    // Basis (G-Anker) mit 1D-Blur (nur Background)
    vec4 gS = applyGaussian1D_Impeller(backgroundTexture, uvBase);

    // NEU: Child Mix (Scharf, unskaliert)
    vec4 cS = texScreen(childTexture, childUVBase + refractionDisplacement);
    gS = mix(gS, cS, cS.a);
...
#if (LG_CA_COMBINE_MODE != 1) // nicht "old only"
    {
        ...
        // Background
        vec4 sG_bg = caSampleAA(backgroundTexture, uvG, aberrUV, sizePx);
        vec4 sR_bg = validR ? caSampleAA(backgroundTexture, uvR, aberrUV, sizePx) : sG_bg;
        vec4 sB_bg = validB ? caSampleAA(backgroundTexture, uvB, aberrUV, sizePx) : sG_bg;

        // NEU: Child (scharf & ohne Blur-Loop, nur mit Refraction + CA Offset)
        vec4 sG_ch = texScreen(childTexture, childUVBase + refractionDisplacement);
        vec4 sR_ch = texScreen(childTexture, childUVBase + refractionDisplacement - aberrUV);
        vec4 sB_ch = texScreen(childTexture, childUVBase + refractionDisplacement + aberrUV);

        if (!validR) sR_ch = sG_ch;
        if (!validB) sB_ch = sG_ch;

        // Composite
        float r = mix(sR_bg.r, sR_ch.r, sR_ch.a);
        float g = mix(sG_bg.g, sG_ch.g, sG_ch.a);
        float b = mix(sB_bg.b, sB_ch.b, sB_ch.a);

        vec3 spectral_new = vec3(r, g, b);
...
#if (LG_CA_COMBINE_MODE != 0) // nicht "new only"
    {
        ...
        if ((ca_old * dispLenPx >= float(LG_CA_VIS_THRESHOLD)) && _shouldApplyCA()) {
            ...
            vec4 sR = _refractAA_CA_OLD_BLURRED(
                backgroundTexture,
                screenUV + dR,
                sizePx,
                dirCAA_old,
            ...
            vec4 sY = _refractAA_CA_OLD_BLURRED(
                backgroundTexture,
                screenUV + dY,
                sizePx,
                dirCAA_old,
            ...
            vec4 sC = _refractAA_CA_OLD_BLURRED(
                backgroundTexture,
                screenUV + dC,
                sizePx,
                dirCAA_old,
            ...
            vec4 sB = _refractAA_CA_OLD_BLURRED(
                backgroundTexture,
                screenUV + dB,
                sizePx,
                dirCAA_old,
            ...
            // NEU: Child Mix
            vec4 cR = texScreen(childTexture, childUVBase + dR);
            sR = mix(sR, cR, cR.a);
            vec4 cY = texScreen(childTexture, childUVBase + dY);
            sY = mix(sY, cY, cY.a);
            vec4 cC = texScreen(childTexture, childUVBase + dC);
            sC = mix(sC, cC, cC.a);
            vec4 cB = texScreen(childTexture, childUVBase + dB);
            sB = mix(sB, cB, cB.a);
            ...
            vec4 sR_aa = _refractAA_CA_OLD_BLURRED(
                backgroundTexture,
                redUV,
                sizePx,
                dirCAA_old,
            ...
            vec4 sB_aa = _refractAA_CA_OLD_BLURRED(
                backgroundTexture,
                blueUV,
                sizePx,
                dirCAA_old,
            ...
            // NEU: Child Mix
            vec4 cR = texScreen(childTexture, childUVBase + (redUV - screenUV) + refractionDisplacement);
            vec4 cB = texScreen(childTexture, childUVBase + (blueUV - screenUV) + refractionDisplacement);
            sR_aa = mix(sR_aa, cR, cR.a);
            sB_aa = mix(sB_aa, cB, cB.a);
            ...
            vec4 cR = texScreen(childTexture, childUVBase + (redUV - screenUV) + refractionDisplacement);
            vec4 cB = texScreen(childTexture, childUVBase + (blueUV - screenUV) + refractionDisplacement);
            red  = mix(red,  cR.r, cR.a);
            blue = mix(blue, cB.b, cB.a);
...
vec4 renderLiquidGlass(
    ...
    // NEU: Übergabe von childTexture und childUVBase
    ...
    if (gStrength > 0.0001 && uTouchCount_f > 0.5) {
        ...
        if (maskRaw > 0.0) {
            ...
            if (extraSigma > 0.01) {
                ...
                // NEU: Child mixen
                vec4 cC = texScreen(childTexture, childUVBase + refractionDisplacement);
