...
// 3) Anzahl zusammengefasster Samples (nach Lerp-Hack)
...
void main() {
  ...
  vec2 pixel = 1.0 / u_size;
  vec2 step_vec = vec2(u_dir.x * pixel.x, u_dir.y * pixel.y);

  vec4 sum = vec4(0.0);
  // Schleife bis 50, aber früh abbrechen anhand u_sample_count.
  for (int i = 0; i < 50; i++) {
    if (float(i) >= u_sample_count) break;
    float t = u_samples[i].x;     // Offset entlang Achse in Pixeln
